...
void QgsQuick3DMapTextureGenerator::render()
{
  ...
  // Use the DEM extent for rendering (same extent as terrain geometry)
  QgsRectangle renderExtent( mExtent.left(), mExtent.top(),
                             mExtent.left() + mExtent.width(),
                             mExtent.top() + mExtent.height() );

  // Find the DEM layer to get its native extent
  QgsRasterLayer *demLayer = nullptr;
  const auto layers = mProject->mapLayers();
  for ( auto it = layers.begin(); it != layers.end(); ++it )
  {
    if ( it.value()->type() == Qgis::LayerType::Raster )
    {
      QgsRasterLayer *rasterLayer = qobject_cast<QgsRasterLayer *>( it.value() );
      if ( rasterLayer && rasterLayer->bandCount() == 1 )
      {
        // Check if this is likely a DEM (single band, used for elevation)
  ...
  // If we have a DEM, use its extent transformed to project CRS
  if ( demLayer )
  {
    QgsCoordinateTransform transform( demLayer->crs(), mProject->crs(), mProject->transformContext() );
