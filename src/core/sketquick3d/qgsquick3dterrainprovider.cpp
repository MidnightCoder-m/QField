...
QVariantList QgsQuick3DTerrainProvider::sampleHeightGrid() const
{
  ...
  // Always use DEM extent (full DEM coverage)
  QRectF extent = demExtent();
  if ( extent.isEmpty() )
  {
  ...
  const double xStep = extent.width() / ( mResolution - 1 );
  const double yStep = extent.height() / ( mResolution - 1 );
  ...
  {
    // QRectF: top() = y = yMinimum (south), bottom() = y+height = yMaximum (north)
    // Start from yMinimum (south) and go north - this matches 3D mesh row order
    const double y = extent.top() + row * yStep;
    for ( int col = 0; col < mResolution; ++col )
    {
      const double x = extent.left() + col * xStep;
